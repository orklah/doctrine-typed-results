...
class EntitiesQuery extends TypedQuery
{
    ...
    public function getResult($hydrationMode = self::HYDRATE_ARRAY)
    {
        ...
        Assertion::allIsInstanceOf($result, $this->type, 'Expected result to be an array of classes instance of ' . $this->type . ' got ' . gettype($result) . '('. (is_string($result) ? $result : '') . ')' . ' instead');
